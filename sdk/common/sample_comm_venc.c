...
/* get stream from each channels and save them */
td_void *sample_comm_venc_get_venc_stream_proc_rtsp(td_void *p)
{
    td_s32 i, ret;
    fd_set read_fds;
    struct timeval timeout_val = {0};
...
/* start get venc stream process thread */
td_s32 sample_comm_venc_start_get_stream_rtsp(ot_venc_chn *venc_chn, td_s32 cnt)
{
    td_s32 i, ret;
    td_s32 wait_times = WAIT_GET_STREAM_THREAD_START_TIMES;
    ...
    ret = pthread_create(&g_venc_pid, 0, sample_comm_venc_get_venc_stream_proc_rtsp, (td_void *)&g_para);
    if (ret < 0) {
        sample_print("create threa failed !!! ret %d\n", ret);
        return TD_FAILURE;
    }
