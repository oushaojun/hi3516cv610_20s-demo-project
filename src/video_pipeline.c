...
td_s32 media_vb_init(const ot_vb_cfg *vb_cfg, td_u32 supplement)
{
    ...
    if (ret != TD_SUCCESS) {
        DBG_ERROR("MEDIA", "ss_mpi_vb_set_cfg failed: 0x%x", ret);
        return ret;
    }
    ...
    if (ret != TD_SUCCESS) {
        DBG_ERROR("MEDIA", "ss_mpi_vb_set_supplement_cfg failed: 0x%x", ret);
        return ret;
    }

    ret = ss_mpi_vb_init();
    if (ret != TD_SUCCESS) {
        DBG_ERROR("MEDIA", "ss_mpi_vb_init failed: 0x%x", ret);
    }
    return ret;
}
...
td_s32 media_vi_start(const sample_vi_cfg *vi_cfg)
{
    ...
    if (ret != TD_SUCCESS) {
        DBG_ERROR("MEDIA", "VI start failed: 0x%x", ret);
    }
    return ret;
}
...
td_s32 media_vpss_start_grp(ot_vpss_grp grp, const media_vpss_grp_attr *attr)
{
    ...
    if (ret != TD_SUCCESS) {
        DBG_ERROR("MEDIA", "VPSS grp%u create failed: 0x%x", grp, ret);
        return ret;
    }

    ret = ss_mpi_vpss_start_grp(grp);
    if (ret != TD_SUCCESS) {
        DBG_ERROR("MEDIA", "VPSS grp%u start failed: 0x%x", grp, ret);
...
td_s32 media_vpss_set_chn(ot_vpss_grp grp, ot_vpss_chn chn, const media_vpss_chn_attr *attr)
{
    ...
    if (ret != TD_SUCCESS) {
        DBG_ERROR("MEDIA", "VPSS grp%u chn%u set attr failed: 0x%x", grp, chn, ret);
    }
    return ret;
}
...
td_s32 media_vpss_enable_chn(ot_vpss_grp grp, ot_vpss_chn chn)
{
    ...
    if (ret != TD_SUCCESS) {
        DBG_ERROR("MEDIA", "VPSS grp%u chn%u enable failed: 0x%x", grp, chn, ret);
    }
    return ret;
}
...
td_void media_vpss_stop_grp(ot_vpss_grp grp, const td_bool *chn_en, td_u32 chn_cnt)
{
    ...
    if (ret != TD_SUCCESS) {
        DBG_ERROR("MEDIA", "VPSS grp%u stop failed: 0x%x", grp, ret);
    }

    ret = ss_mpi_vpss_destroy_grp(grp);
    if (ret != TD_SUCCESS) {
        DBG_ERROR("MEDIA", "VPSS grp%u destroy failed: 0x%x", grp, ret);
    }
}
...
td_s32 media_venc_create(ot_venc_chn chn, const media_venc_chn_attr *attr)
{
    ...
    if (ret != TD_SUCCESS) {
        DBG_ERROR("MEDIA", "unsupported gop mode: %d", attr->gop_mode);
        return TD_FAILURE;
    }
    ...
    if (ret != TD_SUCCESS) {
        DBG_ERROR("MEDIA", "VENC chn%u create failed: 0x%x", chn, ret);
        return ret;
    }
    ...
    /* step 2: set RC param (max_qp/min_qp 等, SDK sample 层未设) */
    {
        ...
        if (ret != TD_SUCCESS) {
            DBG_ERROR("MEDIA", "VENC chn%u get_rc_param failed: 0x%x", chn, ret);
        ...
        if (ret != TD_SUCCESS) {
            DBG_ERROR("MEDIA", "VENC chn%u set_rc_param failed: 0x%x", chn, ret);
            ss_mpi_venc_destroy_chn(chn);
            return ret;
        }
        DBG_LOG("MEDIA", "VENC chn%u RC param configured (mode=%d)", chn, (int)attr->rc_mode);
    }
    ...
    if (attr->type == OT_PT_H264) {
        ...
        if (ret != TD_SUCCESS) {
            DBG_ERROR("MEDIA", "VENC chn%u get_h264_vui failed: 0x%x", chn, ret);
            ss_mpi_venc_destroy_chn(chn);
            return ret;
        }
        ...
        if (ret != TD_SUCCESS) {
            DBG_ERROR("MEDIA", "VENC chn%u set_h264_vui failed: 0x%x", chn, ret);
            ss_mpi_venc_destroy_chn(chn);
            return ret;
        }
        ...
        if (ret != TD_SUCCESS) {
            DBG_ERROR("MEDIA", "VENC chn%u get_h265_vui failed: 0x%x", chn, ret);
            ss_mpi_venc_destroy_chn(chn);
            return ret;
        }
        vui.vui_time_info.timing_info_present_flag = 1;
        ...
        if (ret != TD_SUCCESS) {
            DBG_ERROR("MEDIA", "VENC chn%u set_h265_vui failed: 0x%x", chn, ret);
            ss_mpi_venc_destroy_chn(chn);
            return ret;
        }
    ...
    if (ret != TD_SUCCESS) {
        DBG_ERROR("MEDIA", "VENC chn%u start_chn failed: 0x%x", chn, ret);
        ss_mpi_venc_destroy_chn(chn);
        return ret;
    }
...
td_s32 media_venc_stop(ot_venc_chn chn)
{
    ...
    if (ret != TD_SUCCESS) {
        DBG_ERROR("MEDIA", "VENC chn%u stop failed: 0x%x", chn, ret);
    }
    return ret;
}
...
td_s32 media_venc_destroy(ot_venc_chn chn)
{
    ...
    if (ret != TD_SUCCESS) {
        DBG_ERROR("MEDIA", "VENC chn%u destroy failed: 0x%x", chn, ret);
    }
    return ret;
}
...
td_s32 media_venc_get_frame(ot_venc_chn chn, td_s32 timeout_ms, ot_venc_stream *stream)
{
    ...
    if (fd < 0) {
        DBG_ERROR("MEDIA", "VENC chn%u get_fd failed: 0x%x", chn, fd);
        return TD_FAILURE;
    }
    ...
    if (ret < 0) {
        ...
        DBG_ERROR("MEDIA", "VENC chn%u select failed: errno=%d", chn, errno);
        return TD_FAILURE;
    }
    if (ret == 0) {
        /* 超时无数据 */
    ...
    if (ret != TD_SUCCESS) {
        DBG_ERROR("MEDIA", "VENC chn%u query_status failed: 0x%x", chn, ret);
        return ret;
    }
    if (stat.cur_packs == 0) {
        return MEDIA_ERR_VENC_TIMEOUT;
    ...
    if (stream->pack == TD_NULL) {
        DBG_ERROR("MEDIA", "VENC chn%u malloc pack failed", chn);
        return TD_FAILURE;
    }
    ...
    if (ret != TD_SUCCESS) {
        DBG_ERROR("MEDIA", "VENC chn%u get_stream failed: 0x%x", chn, ret);
        free(stream->pack);
        stream->pack     = TD_NULL;
        stream->pack_cnt = 0;
    }
...
td_s32 media_venc_release_frame(ot_venc_chn chn, ot_venc_stream *stream)
{
    ...
    if (ret != TD_SUCCESS) {
        DBG_ERROR("MEDIA", "VENC chn%u release_stream failed: 0x%x", chn, ret);
    }
                                  ...
                                  sample_vi_cfg *out_vi_cfg)
{
    ...
    if (chn_cnt == 0 || chn_cnt > 4) {
        DBG_ERROR("MEDIA", "invalid chn_cnt: %u", chn_cnt);
        return TD_FAILURE;
    }
    ...
    for (i = 0; i < chn_cnt; i++) {
        ...
        if (ret != TD_SUCCESS) { goto EXIT_VPSS_CLEAN; }
        DBG_LOG("APP", "VPSS chn%u enable OK", i);
    }
    ...
    for (i = 0; i < chn_cnt; i++) {
        ...
        venc_done++;
        DBG_LOG("APP", "VENC chn%u create OK", i);
    }
    ...
    for (i = 0; i < chn_cnt; i++) {
        ...
        DBG_LOG("APP", "VPSS(0,%u) bind VENC(%u) OK", i, i);
    }
...
td_s32 media_pipeline_stream_init(FILE **fps, const td_char **file_names, td_u32 chn_cnt)
{
    ...
    for (i = 0; i < chn_cnt; i++) {
        ...
        if (fps[i] == TD_NULL) {
            DBG_ERROR("APP", "open [%s] failed: %s", file_names[i], strerror(errno));
            /* 关闭已打开的文件 */
            media_pipeline_stream_deinit(fps, i);
            return TD_FAILURE;
        }
        DBG_LOG("APP", "Output chn%u: %s", i, file_names[i]);
    }
    return TD_SUCCESS;
}
...
td_void media_pipeline_stream_deinit(FILE **fps, td_u32 chn_cnt)
{
    ...
    for (i = 0; i < chn_cnt; i++) {
        if (fps[i] != TD_NULL) {
            fclose(fps[i]);
            fps[i] = TD_NULL;
            DBG_LOG("APP", "chn%u file closed", i);
        }
    }
}
...
td_void *media_pipeline_stream_thread(td_void *arg)
{
    ...
    while (!(*targ->exit_flag)) {
        for (chn = 0; chn < targ->chn_cnt; chn++) {
            ...
            if (ret != TD_SUCCESS) {
                DBG_ERROR("MEDIA", "chn%u get_frame error: 0x%x", chn, ret);
                return TD_NULL;
            }
            ...
            if ((frame_cnt[chn] & 0x3F) == 0) {
                DBG_LOG("APP", "chn%u encoded %u frames", chn, frame_cnt[chn]);
            }
        }
        if (targ->chn_cnt > 1) {
            usleep(10000);
